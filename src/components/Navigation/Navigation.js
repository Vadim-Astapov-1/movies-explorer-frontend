...
function Navigation({ loggedIn, handleBtnNavClick }) {
  ...
  return (
    <nav className='navigation'>
      {!loggedIn ? (
        <>
          <Link to='/signup' className='navigation__link-landing'>
            Регистрация
          </Link>
          <Link to='/signin' className='navigation__link-landing navigation__link-ladning_type_btn'>
            Войти
          </Link>
        </>
      ) : (
        <>
          <Link
            ...
            className={`navigation__link ${
              location.pathname === '/' ? 'navigation__link_place_main' : ''
            }`}
          >
            Фильмы
          </Link>
