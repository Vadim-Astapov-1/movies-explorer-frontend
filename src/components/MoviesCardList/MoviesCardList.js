...
function MoviesCardList({
    ...
    return foundMovies
      ...
      .map((item) => (
        <MoviesCard
          ...
          isChecked={handleCheckSaveMovie(item)}
        />
      ));
  }
  ...
  return (
    <div className='movies-cards'>
      <div
        className={`movies-cards__container ${
          isActivePreloader || notFound ? 'movies-cards__container_type_preload' : ''
        }`}
      >
        {makeResult()}
      </div>
      <div className='movies-cards__btn-zone'>
        <button
          className={`movies-cards__btn ${
            foundMovies.length <= countMovies ? 'movies-cards__btn_type_none' : ''
          }`}
